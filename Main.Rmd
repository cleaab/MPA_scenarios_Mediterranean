...
---
### 1. INITIALISATION
# 1.1. Loading libraries and function to read mulitple csv files

```{r}
rm(list=ls())
#-----------------Loading packages-------------------

#install.packages("osmose", repo="https://github.com/osmose-model/drat")

pkgs <- c("tidyverse","here","cowplot","sf","RColorBrewer","ggpubr","raster","terra", "osmose", "ncdf4", "fields", "vegan", "heatmaply", "scales")
...
# Figure 1 - changes in biomass and catch science informed/random
```{r}
...
# Figure A2.2 - cumulated ship density per pixel
```{r}
...
```
# Figure A5.3 - spatial maps of total biomass and catch
```{r}
...
# Figure A6.1 - small pelagics analysis
```{r}
...
# Figure A6.2 - predation pressure
```{r}
...
# Figure A7.1 - alpha-taxonomic divesity inside MPAs before and after their establishment
```{r}
plot.figA7.1(save = TRUE)
```

# Figure A7.2 - alpha-taxonomic divesity and abundance maps before MPA establishment
```{r}
plot.figA7.2(save = TRUE)
```
# Figure A8.1 - heatmap RDA
```{r}

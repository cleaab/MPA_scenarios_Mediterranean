...
# Compute fishing-the-line redistribution for all MPA scenarios
...
for (scen in 1:length(MPA.scenarios.name)){
  for (mpa_coverage in 1:mpa_step){
    ...
    # Calculate border cells
      ...
      Med$effort = rate_vect
      Med$effort_corrected = rate_vect
    ...
    Redistributed_effort_matrix = matrix(Med$effort_corrected, ncol=190, byrow=FALSE)
    sum(Med$effort_corrected)
    mean(Med$effort_corrected)

    # Write csv
    write.table(Redistributed_effort_matrix,
